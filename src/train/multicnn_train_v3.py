"""
train_digit_cnn_v4.py
=====================

v3 실패 원인: AdaptiveAvgPool2d((1,1))이 모든 공간 정보를 압축해서 4자리 분리 학습 불가.

v4 해결: 가로 8칸 보존 + flatten + 자리별 독립 head.
- 모델이 가로축 정보를 유지하면서
- 각 head가 256*8 = 2048차원에서 자기 자리 정보를 선택적으로 사용
- shared head 없이 자리별 독립 (학습 안정성)

실행:
python train_digit_cnn_v4.py \
    --csv /path/train.csv,/path/syn/labels.csv \
    --out_dir ./digit_cnn_runs_v4 \
    --epochs 60 \
    --batch_size 64 \
    --device cuda
"""
...
class DigitCNN(nn.Module):
    ...
    v4: 가로 정보 보존
    - 입력 128x256 (W x H)
    - ConvBlock 4층 → 16x8 (W x H 기준 8x16)
    - AdaptiveAvgPool((1, 8)) — 세로만 압축, 가로 8칸 유지
    - flatten → 256*8 = 2048차원
    - shared FC + 자리별 독립 head
    
    참고: PyTorch는 (B, C, H, W) 형식이라 AdaptiveAvgPool2d((H_out, W_out)).
    여기서 가로 8칸 = W축 8칸 유지하려면 ((1, 8)).
    """
    def __init__(self):
        super().__init__()

        self.backbone = nn.Sequential(
            ConvBlock(3, 32, pool=True),     # H256xW128 -> H128xW64
            ConvBlock(32, 64, pool=True),    # H128xW64  -> H64xW32
            ConvBlock(64, 128, pool=True),   # H64xW32   -> H32xW16
            ConvBlock(128, 256, pool=True),  # H32xW16   -> H16xW8
            # 세로(H)는 1로 압축, 가로(W)는 8칸 그대로
    ...
    def forward(self, x):
        feat = self.backbone(x)           # (B, 256, 1, 8)
...
def main():
    ...
    p.add_argument("--out_dir", type=str, default="./digit_cnn_runs_v4")
